...
for apk in APKS_OF_INTEREST:
    for i in range(0, int(len(unique_bundle_filepaths))):
        for j in range(0, i):
            if i != j:
                result = run_apkdiff(
                    apk_filepath(unique_bundle_filepaths[i], apk),
                    apk_filepath(unique_bundle_filepaths[j], apk),
                )
                # print(i, j, f" :{result}")
                ...
                if result:
                    ...
                    print(
                        f"{apk} matched: {get_folder_run_from_apk_filepath(unique_bundle_filepaths[i])} - {get_folder_run_from_apk_filepath(unique_bundle_filepaths[j])}"
                    )
